        ...
        switch (opTypeId) {
            case OperationTypes_1.OperationTypeId.ExecuteSC: {
                ...
                for (let [key, value] of Object.entries(datastoreKeyMap)) {
                    ...
                    datastoreSerializedBuffer = buffer_1.Buffer.concat([encodedKeyLen, encodedKeyBytes, encodedValueLen, encodedValueBytes]);
                }
                const datastoreSerializedBufferLen = buffer_1.Buffer.from((0, Xbqcrypto_1.varintEncode)(datastoreSerializedBuffer.length));
                if (datastoreSerializedBuffer.length === 0) {
                    return buffer_1.Buffer.concat([feeEncoded, expirePeriodEncoded, typeIdEncoded, maxGasEncoded, gasPriceEncoded, dataLengthEncoded, contractDataEncoded, datastoreSerializedBufferLen]);
                }
                return buffer_1.Buffer.concat([feeEncoded, expirePeriodEncoded, typeIdEncoded, maxGasEncoded, gasPriceEncoded, dataLengthEncoded, contractDataEncoded, datastoreSerializedBufferLen, datastoreSerializedBuffer]);
            }
